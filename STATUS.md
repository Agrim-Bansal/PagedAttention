...
A content review then listed a handful of factual/consistency fixes.
...
- **Scenes** (file / class / slides / figures):
  ...
  | s4_problem.py | S4Problem | 20 | 2, 3 |
  | s5_pagedattention.py | S5PagedAttention | 17 | 5 (run live), 6 (+1 step), 7, 2 (vLLM bar) |
...
- **S5 rewritten from scratch (2026-09-05, 17 beats)** — see `PLANNING_NOTES.md` decision
  17 and the S5 paragraph in `TALK_PLAN.md`. Rendered at `-q h`, all 18 resting frames
  inspected. Also fixed a z-order bug that made every filled cell's word label invisible
  (the old "crossfade" helper): `AnimationGroup` must get `group=<on-screen parent>`;
  documented in `talk/SCENE_BRIEF.md`. Note for QA: `tools/last_frames.py` uses
  `-sseof -0.05`, which misses the last frame of 15 fps `-q l` renders and falls back to
  frame 0 (looks blank / one beat behind); it is fine for 60 fps `-q h` output.
...
7. ~~Spot-check S5~~ done with the S5 rewrite (S5 is rendered at `-q h`).

Rebuild:
8. Re-render every changed scene:
   `.venv/bin/manim-slides render -q h talk/<file>.py <Class>` (S0 now;
   plus S3, S7, S9, S10 after the fixes above. S5 is already re-rendered.)
